$(function () {
    /*数据表格*/
    $("#menu_datagrid").datagrid({
        url: "/menuList",
        columns: [[
            {field: 'text',title:'名称' ,width: 1, align: 'center'},
            {field: 'url', title:'跳转地址' ,width: 1, align: 'center'},
            {field: 'parent',title:'父菜单' , width: 1, align: 'center', formatter: function (value, row, index) {
                return value ? value.text : '';
                }
            }
        ]],
        fit: true,
        rownumbers: true,
        singleSelect: true,
        striped: true,
        pagination: true,
        fitColumns: true,
        toolbar: '#menu_toolbar'
    });

    /*初始化新增/编辑对话框 */
    $("#menu_dialog").dialog({
        width: 300,
        height: 300,
        closed: true,
        buttons: '#menu_dialog_bt'
    });

    /*获取全部的父菜单*/
    $("#parentMenu").combobox({
        width:150,
        panelHeight:'auto',
        editable:false,
        url:'parentList',
        textField:'text', /*显示的名称*/
        valueField:'id',
        onLoadSuccess:function(){/*数据加载完毕后回调*/
            /*在刷新的时候input里面的placeholder就会不在  需要重新设置*/
            $("#parentMenu").each(function(i){
                var span = $(this).siblings("span")[i];
                var targetInput = $(span).find("input:first");
                if(targetInput){
                    $(targetInput).attr("placeholder", $(this).attr("placeholder"));
                }
            });
        }
    });

    /*添加菜单*/
    $("#add").click(function () {
        $("#menu_dialog").dialog("setTitle","增加菜单"); /*设置标题*/
        $("#menu_form").form("clear");
        $("#menu_dialog").dialog("open");
    });

    /*保存*/
    $("#save").click(function () {
        var id = $("[name='id']").val();  /*判断当前是添加 还是编辑*/
        var url;
        if(id){
            var parent_id = $("[name='parent.id']").val();
            if(id == parent_id){
                $.messager.alert("温馨提示","不能设置自己为父菜单");
                return;
            }
            url = "updateMenu";  /*更新*/
        }else{
            url = "saveMenu";  /*添加*/
        }
        $("#menu_form").form("submit",{ /*提交表单*/
            url: url,
            success:function (data) {/*提交返回的信息*/
                data = $.parseJSON(data);/*解析成json*/
                if(data.success){
                    $.messager.alert("温馨提示：",data.msg);
                    $("#menu_dialog").dialog("close");/*关闭对话框*/
                    $("#parentMenu").combobox("reload");/*刷新下拉菜单值*/
                    $("#menu_datagrid").datagrid("reload"); /*重新加载表格*/
                }else {
                    $.messager.alert("温馨提示：",data.msg);
                }
            }
        });
    });

    /*取消列表框*/
    $("#cancel").click(function () {
        $("#menu_dialog").dialog("close");
    });

    /*编辑菜单*/
    $("#edit").click(function () {
        $("#menu_form").form("clear"); /*清空表单数据*/
        var rowData = $("#menu_datagrid").datagrid("getSelected"); /*获取当前选中的行*/
        if(!rowData){
            $.messager.alert("提示","选择一行数据进行编辑");
            return;
        }
        /*父菜单回显*/
        if(rowData.parent){
            rowData["parent.id"] = rowData.parent.id;
        }else{ /*回显的placeholder*/
            $("#parentMenu").each(function (i) {
                var span = $(this).siblings("span")[i];
                var targetInput = $(span).find("input:first");
                if(targetInput){
                    $(targetInput).attr("placehoder",$(this).attr("placehoder"));
                }
            });
        }
        $("#menu_dialog").dialog("open"); /*弹出对话框*/
        $("#menu_dialog").dialog("setTitle","编辑菜单"); /*设置标题*/
        $("#menu_form").form("load",rowData); /*选中数据的回显 */
    });

    /*删除菜单*/
    $("#del").click(function () {
        var rowData = $("#menu_datagrid").datagrid("getSelected"); /*获取当前选中的行*/
        if(!rowData){
            $.messager.alert("提示","选择一行数据进行删除");
            return;
        }
        $.messager.confirm("确认","是否做删除操作",function (res) { /*提醒用户，是否做删除操作*/
            if(res){
                $.get("/deleteMenu?id="+rowData.id,function (data) {
                    if(data.success){
                        $.messager.alert("温馨提示：",data.msg);
                        $("#menu_datagrid").datagrid("reload");      /*重新加载表格*/
                        $("#parentMenu").combobox("reload");/*刷新下拉菜单值*/
                    }else {
                        $.messager.alert("温馨提示：",data.msg);
                    }
                })
            }
        })
    });












});
